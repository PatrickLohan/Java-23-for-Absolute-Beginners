...
/**
 * @author iuliana.cosmina on 03/07/2024
 */
public class DerbyDBConnection implements DbConnection {
    ...
    public Account insert(Account account) {
        try {
            ...
            stmt.execute("insert into " + tableName + "(holder, number, amount)"
                    + " values ('" + account.holder() + "','" + account.number() + "'," + account.sum() + ")");
            stmt.close();
    ...
    public Account findByHolder(String holder) {
        try {
            Account acc = null;
            stmt = conn.createStatement();
            var results = stmt.executeQuery("select * from " + tableName + " where holder='" + holder + "'");
            while (results.next()) {
                var holderdb = results.getString(1);
                var number = results.getString(2);
                var amount = results.getInt(3);
    ...
    public List<Account> findAll() {
        try {
            ...
            while (results.next()) {
                ...
                var number = results.getString(2);
                var amount = results.getInt(3);
                var acc = new Account(holderdb, amount, number);
                accList.add(acc);
            }
            results.close();
            stmt.close();
    ...
    public Account update(Account account) {
        try {
            ...
            stmt.execute("update " + tableName
                    + " set amount=" + account.sum()
                    + " where holder='" + account.holder() + "'");
            stmt.close();
        } catch (SQLException e) {
    ...
    public void delete(String holder) {
        try {
            stmt = conn.createStatement();
            stmt.execute("delete from " + tableName + " where holder='" + holder + "'");
            stmt.close();
        } catch (SQLException e) {
